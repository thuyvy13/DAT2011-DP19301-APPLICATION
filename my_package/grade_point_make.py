from grade_point_check import get_subject_code, get_type_choice, get_number ,check_user , thoat_chuong_trinh
...
def display_scores():
    ...
    for item in score_items:
        print(f"{item['Mã']:<15} {item['Tên']:<20} {item['Trọng số (%)']:<15} {item['Môn học']:<15} {item['Phân loại']}")
...
def add_score():
    while True :
        ...
        if type_choice == "quiz":
            ...
            while any(item['Mã'] == ma_diem and item['Môn học'] == mon_hoc for item in score_items):
                print(f"Assignment {assignment_number} đã tồn tại trong môn {mon_hoc}. Vui lòng nhập một assignment khác.")
    ...
    while True :
        ...
        if matched_subjects:
            ...
            for subject in matched_subjects:
                print(f"{subject['Mã']:<15} {subject['Tên']:<20} {subject['Trọng số (%)']:<15} {subject['Môn học']:<15} {subject['Phân loại']}")
        else:
    ...
    while True : 
        ...
        while True:
            ...
            if type_choice in ["quiz", "lab", "assignment"]:
                existing_items = [item for item in score_items if item['Môn học'] == chosen_subject and type_choice.capitalize() in item['Tên']]
                if existing_items:
                    break
                else:
        ...
        while True:
            quiz_number_str = input(f"Nhập số thứ tự {type_choice} bạn muốn sửa (1-4 cho Quiz, 1-8 cho Lab, 1-3 cho Assignment): ")
            try:
        ...
        if any(item['Mã'] == ma_diem and item['Môn học'] == chosen_subject for item in score_items):
            item_to_update = next(item for item in score_items if item['Mã'] == ma_diem and item['Môn học'] == chosen_subject)
        else:
        ...
        while True:
            new_number_str = input(f"Nhập số thứ tự mới cho {item_to_update['Tên']} (nếu là Quiz, 1-4; nếu là Lab, 1-8; nếu là Assignment, 1-3): ")
            try:
                new_number = int(new_number_str)
                if (type_choice == "quiz" and 1 <= new_number <= 4) or (type_choice == "lab" and 1 <= new_number <= 8) or (type_choice == "assignment" and 1 <= new_number <= 3):
...
def find_highest_score():
    ...
    # Liệt kê các môn học có trong danh sách
        ...
        if highest_score_item:
            ...
            print(f"Mã: {highest_score_item['Mã']}, Tên: {highest_score_item['Tên']}, Trọng số: {highest_score_item['Trọng số (%)']}%, Môn học: {highest_score_item['Môn học']}, Phân loại: {highest_score_item['Phân loại']}")
        else:
...
def Thong_ke():
    ...
    while True:
        ...
        for subject in subjects:
            ...
            current_assignment_count = sum(1 for item in score_items if item['Môn học'] == subject and 'ASS' in item['Mã'])
            ...
            print(f"\nMôn học: {subject}")
            print(f"Số lượng quiz hiện có: {current_quiz_count}, cần thêm: {quizzes_needed if quizzes_needed > 0 else 0}")
            print(f"Số lượng lab hiện có: {current_lab_count}, cần thêm: {labs_needed if labs_needed > 0 else 0}")
            print(f"Số lượng assignment hiện có: {current_assignment_count}, cần thêm: {assignments_needed if assignments_needed > 0 else 0}")
